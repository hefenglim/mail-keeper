...
def test_engine_requires_quoting_and_decodes_mutf7_mailbox():
    # 保真（F3）：信箱名須為 atom 或 quoted-string——未加引號含空白 → 引擎拒（真伺服器亦拒），
    # 故引擎能抓出「外送夾名未加引號」的產品 bug（F2）；quoted + mUTF-7 則正確還原（F1）。
    server = ImapServer({"INBOX": [], "Junk Email": [message(10, "x")], "台北": [message(20, "y")]})
    assert _imap_over(server).select('"Junk Email"')[0] == "OK"        # quoted 多字夾名 → OK
    assert _imap_over(server).select('"&U,BTFw-"')[0] == "OK"          # quoted mUTF-7（台北）→ 解碼比對 → OK
    with pytest.raises(imaplib.IMAP4.error):                           # 未加引號含空白 → BAD（imaplib 拋）
        _imap_over(server).select('Junk Email')
