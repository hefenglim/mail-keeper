...
def execute(backend: MailBackend, items: list[ReportItem]) -> list[MoveResult]:
    ...
    for it in candidates(items):
        ...
        try:
            ...
            present = {h.uid for h in backend.list_headers(row.current_folder)}
