...
- ✅ **殘留收斂（SR 條件）**：合規折行器再硬化——頭段恆含「欄名 + 首個**非空**值詞」（即使值以空白
  起始、含連續空白），保證值**永不落續行**，徹底消除「值前導空白 → 續行起始 → 3.10 版本差」的潛在
  矩陣-only failure（先前母版碰不到、屬潛伏）。
- ⚠ **已知產品限制（誠實標注，SR 條件）**：`malformed_fold` 測試的 `.strip()` 是**承重**的——不合規
  折行下，**3.10** 產品會殘留前導折疊空白（`_unfold` 只摺疊含換行的續行，不去 email 攤平後殘留的前導
