...
## D4 — 候選分組（P4）：**本期延後（Deferred to P2/P3）**

- **Decision**: 本期**不做** P4 候選分組；`execute`／搬移路徑維持現狀（CSV/候選順序處理與輸出）。
- **Rationale（分析後）**: 本期未改 move 路徑（P2 批次搬移、P3 免重複 SELECT 皆延後），因此分組**不產生任何往返削減**——每封仍各自 `SELECT`+`UID MOVE`。分組此期唯一可觀察效果只剩「早停邊界行為改變」：`execute` 的「連續失敗達門檻早停」對處理順序敏感，分組會使同夾失敗群聚、改變早停下被處理的候選集合（happy path 不受影響，但會破壞 SC「搬移結果集合 100% 一致」於早停邊界）。**全成本、零收益、且引入行為風險** → 不宜本期單放。
